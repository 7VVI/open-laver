      ...
      <div className="max-w-4xl mx-auto px-8 py-8">
        ...
        <div className="mt-5 relative max-w-md">
          ...
          <input
            ...
            placeholder="搜索定时任务..."
            className="w-full bg-white border border-slate-300 rounded-lg pl-9 pr-3 py-2 text-sm text-slate-700 focus:outline-none focus:border-[#34c759]"
          />
        </div>
        ...
        <div className="mt-4 flex items-center justify-between bg-blue-50 border border-blue-100 rounded-lg px-4 py-2.5 text-sm">
          ...
          <label className="flex items-center gap-2 cursor-pointer text-slate-600">
            ...
            <span
              ...
              className={`w-9 h-5 rounded-full relative transition ${keepAwake ? "bg-[#34c759]" : "bg-slate-300"}`}
            >
          ...
          <div className="grid grid-cols-3 gap-4">
            {EXAMPLES.map((ex) => (
              <button
                ...
                className="text-left bg-white border border-slate-200 rounded-xl p-4 hover:border-[#34c759] hover:shadow-sm transition"
              >
        ...
        <div className="mt-8 flex items-center gap-5 border-b border-slate-200">
          <button
            onClick={() => setTab("jobs")}
            className={`pb-2 text-sm font-medium border-b-2 -mb-px ${tab === "jobs" ? "border-[#34c759] text-slate-800" : "border-transparent text-slate-400 hover:text-slate-600"}`}
          >
            我的定时任务
          </button>
          <button
            onClick={() => setTab("runs")}
            className={`pb-2 text-sm font-medium border-b-2 -mb-px ${tab === "runs" ? "border-[#34c759] text-slate-800" : "border-transparent text-slate-400 hover:text-slate-600"}`}
          >
            执行记录
          </button>
        </div>
                ...
                <div key={j.id} className="bg-white border border-slate-200 rounded-xl p-4 flex items-start justify-between gap-4 shadow-sm">
                  <div className="min-w-0">
                    ...
                    <div className="flex items-center gap-2 mt-2">
                      <span className="inline-flex items-center gap-1 text-xs text-[#1a7f37] bg-[#e8f8ee] rounded px-2 py-0.5">
                        <svg viewBox="0 0 24 24" className="w-3 h-3" fill="none" stroke="currentColor" strokeWidth="1.8"><path d="M12 8v4l3 2" /><circle cx="12" cy="12" r="9" /></svg>
                        {describeCron(j.expr)}
                      </span>
              ...
              <div className="space-y-2">
                {runs.map((r, i) => (
                  <div key={i} className="bg-white border border-slate-200 rounded-lg px-4 py-3 flex items-center justify-between gap-4">
                    ...
                    <span className={`text-[10px] px-2 py-0.5 rounded shrink-0 ${r.trigger === "manual" ? "bg-amber-50 text-amber-600" : "bg-[#e0e0e0] text-[#333333]"}`}>
...
function CronCreateDialog({
  ...
  const field = "bg-white border border-slate-300 rounded-lg px-3 py-2 text-sm text-slate-800 focus:outline-none focus:border-[#34c759]";
