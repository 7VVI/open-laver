            ...
            {showConfig && (
              <div className="p-5 border-b border-slate-100 bg-slate-50/60">
                ...
                <textarea
                  ...
                  rows={10}
                  spellCheck={false}
                  className="w-full font-mono text-xs bg-white border border-slate-200 rounded-lg p-3 text-slate-700 focus:outline-none focus:border-[#34c759]"
                />
                <div className="flex items-center gap-3 mt-2">
                  <button
                    onClick={save}
                    className="bg-[#333333] hover:bg-[#111111] text-white rounded-lg px-4 py-2 text-sm"
                  >
                    保存并连接
                  </button>
                  ...
                  {servers.map((s) => (
                    <div key={s.name} className="bg-white border border-slate-200 rounded-lg p-3">
                      <div className="flex items-center gap-2">
                        <span className={s.connected ? "text-green-600" : "text-red-400"}>
                          {s.connected ? "●" : "○"}
                        </span>
